...
#'
#' @description A shiny Module for the Genome tab, showing gene positions on
#'   chromosomes with an interactive plotly view and optional sliding window
#'   enrichment analysis.
#'
...
#' Static genome plot modal UI — placed outside tabsetPanel in mainPanel
mod_09_genome_modal_ui <- function(id) {
  ns <- shiny::NS(id)
...
#' @param converted               Reactive returning gene ID conversion result
...
mod_09_genome_server <- function(id, sidebar_values, geneInfoLookup, converted,
        ...
        withProgress(message = sample(quotes, 1), detail = "Visualzing expression on the genome", {
          ...
          if (dim(x)[1] > 5) {
            ...
            if (dim(x)[1] > 5) {
              ...
              movingAverage <- genome_sliding_window(
                x0, listN, totalN, windowSize, steps,
                pvalCutoff = as.numeric(input$chRegionPval),
                chD = chD
              )
